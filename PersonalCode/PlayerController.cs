...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.DrawRay(_boundsBottomLeft , Vector3.left , Color.green);
        Debug.DrawRay(_boundsBottomRight, Vector3.right , Color.green);
        Debug.DrawRay(_boundsTopLeft , Vector3.left , Color.green);
        Debug.DrawRay(_boundsTopRight, Vector3.right, Color.green);
    ...
    private void CollisionBelow()
    {
        ...
        //calculation ray length
        float rayLength = (_boundsHeight / 2) + _skin;
        if(_movePosition.y <0)
        {
            rayLength += Mathf.Abs(_movePosition.y);
        }

        //calculating ray origin
        Vector2 leftOrigin = (_boundsBottomLeft + _boundsTopLeft) / 2f;
        Vector2 rightOrigin = (_boundsBottomRight + _boundsTopRight) /2f;

        leftOrigin += (Vector2)(transform.up * _skin) + (Vector2)(transform.right * _movePosition.y);

        rightOrigin += (Vector2)(transform.up * _skin) + (Vector2)(transform.right * _movePosition.y);
        ...
        for(int i=0;i<verticalRayAmount;i++)
        {
            ...
            if(hit)
            {
                ...
                {
                    Debug.Log("MovingPlatform Working!");
        ...
        for(int i=0 ; i<horizontalRayAmount ; i++)
        {
            ...
            if(hit)
            {
                ...
                if(direction >= 0)
                {
                    ...
                    Debug.Log("WE hit right object");
                }
                else
                {
                    _movePosition.x = -hit.distance + _boundsWidth /2f -_skin * 2f;
                    _conditions.IsCollidingLeft = true;
                    Debug.Log("WE hit left object");
                }
                _force.x=0f;
    ...
    private void CollisionAbove()
    {
        ...
        Vector2 rayTopLeft = (_boundsTopLeft + _boundsBottomLeft) / 2f;
        Vector2 rayTopRight = (_boundsTopRight + _boundsBottomRight) / 2f;
    ...
    private void EnterMovingPlatform()
    {
        ...
        if(_movingPlatform.CollidingWithPlayer)
        {
            ...
            Vector3 moveDirection = _movingPlatform.Direction == PathFollow.MoveDirection.RIGHT?Vector3.right:Vector3.left; 
     ...
     private void SetRayOrigins()
    {
        ...
        _boundsHeight = Vector2.Distance(_boundsTopLeft,_boundsBottomLeft);
